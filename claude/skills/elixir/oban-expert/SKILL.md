...
- **`unique` keys must be JSON-encodable** — atoms in args become strings on the wire. `unique: [keys: [:user_id]]` matches against `%{"user_id" => 123}` after JSON round-trip; mismatched key types silently produce duplicates. Defensive practice: use string keys in args everywhere (`%{"user_id" => id}`, not `%{user_id: id}`), or normalize inside the worker's own `new/1` builder so test and production paths can't diverge.
- **`{:snooze, n}` doesn't increment attempt count** — useful when retrying isn't a failure (rate limits, downstream maintenance windows). But snoozed jobs can theoretically loop forever; cap it explicitly: compare `job.scheduled_at` to `job.inserted_at` and `{:discard, "snoozed too long"}` past a threshold, or just use `{:error, :rate_limited}` and let `max_attempts` + backoff handle it.
...
- **The `Lifeline` plugin rescues "stuck" executing jobs** — jobs marked `:executing` whose worker died without updating state get reset to `:available` after `rescue_after`. Crucial for crashes; can mask real bugs (resource deadlocks, external system hangs) that leave jobs stuck legitimately. Monitor the rate of Lifeline rescues via telemetry — sustained rescue activity means something downstream is broken, not just crashing.
